/**
 * ============================================================================
 * SITE CONFIG — A ÚNICA FONTE DE VERDADE DOS DADOS DA LOJA
 * ============================================================================
 *
 * Todo dado que muda de uma Loja para outra vive AQUI. Nenhum outro arquivo
 * do site público precisa ser tocado para lançar a versão de uma nova Loja.
 *
 * Como isso chega no HTML:
 *   Os arquivos .html contêm marcadores {{loja.nome}} e blocos
 *   <!-- @each veneraveis --> ... <!-- @endeach -->
 *   O plugin em `vite-plugin-site-config.js` substitui tudo NO BUILD,
 *   gerando HTML estático puro (bom para SEO e para o preview do WhatsApp).
 *
 * REGRA DAS IMAGENS:
...
export default {
  ...
  loja: {
    ...
    nomeCompleto: "A∴R∴L∴S∴ Major Manoel dos Santos Portugal Nº 4424",
    // Versão curta, usada no cabeçalho em telas pequenas.
    nomeCurto: "Major Manoel",
    numero: "4424",
    // Aparece sob o nome no rodapé.
    obediencia: "GOB - Rito Moderno",
    rito: "Rito Moderno",
    // Complemento do <title>. O título final é montado como:
    //   "<nomeCompleto> - <slogan>"  — o nome NÃO se repete aqui, para não
    //   existir dois lugares com o nome da Loja podendo divergir.
  ...
  // ---------------------------------------------------------------------------
  contato: {
    email: "contato@majorportugal.org.br",
    endereco:
  ...
  // ---------------------------------------------------------------------------
  // INTEGRAÇÕES
  // A publicKey do EmailJS é pública por natureza (roda no navegador).
  // Já as chaves do Supabase continuam em `public/js/supabase-config.js`.
  // ---------------------------------------------------------------------------
  integracoes: {
    emailjs: {
    ...
    // -------------------------------------------------------------------------
    // SUPABASE — UM PROJETO POR LOJA
    //
    // Cada Loja tem seu PRÓPRIO projeto Supabase. É isso que torna o vazamento
    // entre Lojas impossível: não existe tabela compartilhada de onde vazar.
    // Ao cadastrar a Loja B, crie um projeto novo no Supabase e cole aqui a URL
    // e a chave publicável DELA.
    //
    // A chave publicável (anon) é pública por natureza — ela vai para o
    // navegador de qualquer visitante. Ela NÃO é o que protege seus dados;
    // quem protege é o RLS (ver supabase/01_rls.sql). NUNCA coloque aqui a
    // chave `service_role`: ela ignora o RLS e daria acesso total ao banco.
    // -------------------------------------------------------------------------
    supabase: {
      url: "https://dnemzaksujqnywbnfamf.supabase.co",
      anonKey: "sb_publishable_lB9tBZKn3oa9pQykI22ePw_m6FNlq84",
    },
  },
};
